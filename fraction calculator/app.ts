...
function substraction(a: Fraction , b: Fraction): Fraction {
	let difference : Fraction;
	a.numerator *= b.denominator;
	b.numerator *= a.denominator;
	let commonDenominator = a.denominator * b.denominator;
	difference.numerator = a.numerator - b.numerator;
	difference.denominator = commonDenominator;
...
function division(a: Fraction , b: Fraction): Fraction {
	if (b.numerator == 0)
		throw("MATH ERROR!! impossible to divide by 0.")
	else{
...
// the following function returns the greates common dividor of two integers given as parameters
...
function calcul(){
	...
	if (a.denominator == 0 || b.denominator == 0)
		...
		switch((<HTMLInputElement>document.getElementById("operation")).value){
			case "+":{
				result = addition(a,b);
				(<HTMLInputElement>document.getElementById("resultNumerator")).value = result.numerator.toString();
				(<HTMLInputElement>document.getElementById("resutDenominator")).value = result.denominator.toString();
			}
			case "-":{
